...
`avoidance.strategy` supports `disabled`, `scripted`, and `tangentbug_depth`. The depth planner reduces a configured image band to a smoothed column profile, finds free-space gaps and depth discontinuities, scores local headings using target direction and clearance, and emits one incremental motion decision. It is a local reactive planner, not metric localization or a complete global TangentBug implementation.
